...
本版本继续补齐中文界面：首次启动引导、权限授权说明、软件更新、媒体控制布局、媒体来源和模式选项、文件架空状态、文件架操作菜单、打开方式面板、重命名面板、图片转换弹窗和图片处理错误提示已改为更适合中文用户阅读的展示文案。
...
if gh release view "$TAG" --repo "$REPO_FULL_NAME" >/dev/null 2>&1; then
  gh release edit "$TAG" --repo "$REPO_FULL_NAME" --title "Boring Notch CN 2.7.3-cn.10" --notes-file "$RELEASE_NOTES"
  gh release upload "$TAG" "$DMG_PATH" --repo "$REPO_FULL_NAME" --clobber
else
  gh release create "$TAG" "$DMG_PATH" --repo "$REPO_FULL_NAME" --title "Boring Notch CN 2.7.3-cn.10" --notes-file "$RELEASE_NOTES"
fi
