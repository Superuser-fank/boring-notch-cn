...
Boring Notch CN 是 Boring Notch 的公开源码 fork，主品牌名不使用任何第三方服务名称。本版本增加了中文设置本地化、菜单栏和刘海快捷菜单中文化、媒体诊断、诊断信息复制和中文建议处理步骤、权限与安装自检、关于页下载与安装入口、网易云音乐和 QQ 音乐支持，补充中文月份/星期、农历、常见中国节日、2026 年中国法定休假和补班提示，并为文件架快速分享增加微信、QQ、钉钉、飞书快捷投递。

本版本继续优化中国用户日常可见界面：刘海右键菜单、刘海快捷菜单、相机权限弹窗、电池弹层、HUD、下载提示和备用状态栏菜单进一步中文化；关于页保留“下载与安装”，可直接打开 GitHub Release 下载页，也可复制解除 macOS Gatekeeper 拦截的 quarantine 命令。媒体诊断会根据播放器是否运行、macOS 正在播放数据是否来自所选来源、是否拿到歌曲数据，给出中文建议处理步骤；“复制诊断信息”也会带上这些建议，便于反馈 GitHub issue。诊断文本包含 App/系统版本、媒体来源、播放器状态、正在播放数据状态、控制能力和关键权限状态，但不包含歌曲名、歌手名、联系人或文件内容。
...
- 对应源码：<https://github.com/Superuser-fank/boring-notch-cn/tree/v2.7.3-cn.15>
...
if gh release view "$TAG" --repo "$REPO_FULL_NAME" >/dev/null 2>&1; then
  gh release edit "$TAG" --repo "$REPO_FULL_NAME" --title "Boring Notch CN 2.7.3-cn.15" --notes-file "$RELEASE_NOTES"
  gh release upload "$TAG" "$DMG_PATH" --repo "$REPO_FULL_NAME" --clobber
else
  gh release create "$TAG" "$DMG_PATH" --repo "$REPO_FULL_NAME" --title "Boring Notch CN 2.7.3-cn.15" --notes-file "$RELEASE_NOTES"
fi
