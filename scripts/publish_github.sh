...
Boring Notch CN 是 Boring Notch 的公开源码 fork，主品牌名不使用任何第三方服务名称。本版本增加了中文设置本地化、媒体诊断和诊断信息复制、权限与安装自检、网易云音乐和 QQ 音乐支持，补充中文月份/星期、农历、常见中国节日、2026 年中国法定休假和补班提示，并为文件架快速分享增加微信、QQ、钉钉、飞书快捷投递。

本版本继续优化中国用户排查体验：媒体设置页新增“复制诊断信息”，会生成中文状态文本，包含 App/系统版本、媒体来源、播放器状态、正在播放数据状态、控制能力和关键权限状态，便于反馈 GitHub issue；诊断文本不包含歌曲名、歌手名、联系人或文件内容。通用设置页的“权限与安装自检”、首次启动引导、权限授权说明、软件更新、媒体控制布局、媒体来源和模式选项、文件架菜单与图片处理弹窗也保持中文展示。
...
- 对应源码：<https://github.com/Superuser-fank/boring-notch-cn/tree/v2.7.3-cn.12>
...
if gh release view "$TAG" --repo "$REPO_FULL_NAME" >/dev/null 2>&1; then
  gh release edit "$TAG" --repo "$REPO_FULL_NAME" --title "Boring Notch CN 2.7.3-cn.12" --notes-file "$RELEASE_NOTES"
  gh release upload "$TAG" "$DMG_PATH" --repo "$REPO_FULL_NAME" --clobber
else
  gh release create "$TAG" "$DMG_PATH" --repo "$REPO_FULL_NAME" --title "Boring Notch CN 2.7.3-cn.12" --notes-file "$RELEASE_NOTES"
fi
