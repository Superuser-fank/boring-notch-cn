...
Boring Notch CN 是 Boring Notch 的公开源码 fork，主品牌名不使用任何第三方服务名称。本版本增加了中文设置本地化、媒体诊断、权限与安装自检、网易云音乐和 QQ 音乐支持，补充中文月份/星期、农历、常见中国节日、2026 年中国法定休假和补班提示，并为文件架快速分享增加微信、QQ、钉钉、飞书快捷投递。

本版本继续优化中国用户上手体验：通用设置页新增“权限与安装自检”，可直接查看辅助功能、日历、提醒事项、相机权限状态，并提供 Gatekeeper 解除隔离命令、权限请求和系统设置入口。此前补齐的首次启动引导、权限授权说明、软件更新、媒体控制布局、媒体来源和模式选项、文件架菜单与图片处理弹窗也保持中文展示。
...
if gh release view "$TAG" --repo "$REPO_FULL_NAME" >/dev/null 2>&1; then
  gh release edit "$TAG" --repo "$REPO_FULL_NAME" --title "Boring Notch CN 2.7.3-cn.11" --notes-file "$RELEASE_NOTES"
  gh release upload "$TAG" "$DMG_PATH" --repo "$REPO_FULL_NAME" --clobber
else
  gh release create "$TAG" "$DMG_PATH" --repo "$REPO_FULL_NAME" --title "Boring Notch CN 2.7.3-cn.11" --notes-file "$RELEASE_NOTES"
fi
